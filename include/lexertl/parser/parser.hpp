    ...
    namespace detail
    {
        ...
        */
        ...
        public:
            ...
            typedef basic_re_tokeniser<rules_char_type, input_char_type,
                id_type> tokeniser;
        ...
        private:
            ...
            struct find_functor
            {
                // Pointer to stop warning about cannot
            ...
            void create_sequence(std::auto_ptr<token>& token_)
            {
                ...
                typename token::string_token::range_vector::const_iterator
                    end_ = token_->_str._ranges.end();
            ...
            void slice_range(const input_range& range_,
                string_token_vector data_[2], const false_&)
            {
            ...
            // 24 bit unicode
            void slice_range(const input_range& range_,
                string_token_vector data_[3], const true_&)
            {
                const unsigned char first_msb_ = static_cast<unsigned char>
                ...
                const unsigned char second2_, string_token_vector data_[2])
            {
                ...
                const string_token token2_(first2_ > second2_ ?
                    second2_ : first2_, first2_ > second2_ ?
                    first2_ : second2_);
                ...
                const unsigned char second3_, string_token_vector data_[3])
            {
                ...
                const string_token token3_(first3_ > second3_ ?
                    second3_ : first3_, first3_ > second3_ ?
                    first3_ : second3_);
            ...
            void optional(const bool greedy_)
            {
                ...
                for (typename node::node_vector::iterator iter_ =
                    firstpos_.begin(), end_ = firstpos_.end(); iter_ != end_;
                    ++iter_)
                {
            ...
            void fixup_bol(node*& root_)const
            {
                ...
                for (; iter_ != end_; ++iter_)
                {
                    ...
                    found_ = !node_->end_state() &&
                        node_->token() == bol_token();
                ...
                if (!found_)
                {
                    ...
                    _node_ptr_vector->push_back(static_cast
                        <selection_node*>(0));
                    _node_ptr_vector->back() = new selection_node(lhs_, rhs_);
