...
namespace lexertl
{
    template<typename rules, typename sm, typename char_traits =
        basic_char_traits<typename sm::traits::input_char_type> >
    ...
    public:
            ...
            for (id_type index_ = 0; index_ < size_; ++index_)
            {
                ...
                else
                {
                    ...
                    node* root_ = build_tree(rules_, index_, node_ptr_vector_,
                        charset_map_, cr_id_, nl_id_, unique_id_);
        ...
        static node* build_tree(const rules& rules_, const std::size_t dfa_,
            ...
            id_type& cr_id_, id_type& nl_id_, id_type& unique_id_)
        {
            ...
            while (regex_iter_ != regex_iter_end_)
            {
                ...
                node* rhs_ = parser_.parse(re_, *id_iter_, *user_id_iter_,
                    ++unique_id_, *next_dfa_iter_, *push_dfa_iter_,
                    *pop_dfa_iter_, cr_id_, nl_id_,
    ...
    protected:
        ...
        typedef typename rules::std_string_deque std_string_deque;
        ...
        {
            typename node_vector::iterator iter_ = followpos_.begin();
            typename node_vector::iterator end_ = followpos_.end();
            ...
            const id_type nl_id_, const index_set_vector& set_mapping_)
        {
            ...
            while (nl_iter_ != end_)
            {
                ...
                const index_set& cr_set_ =
                    set_mapping_[node_->token()];

                if (cr_set_.find(cr_id_) != cr_set_.end())
                {
                    const node_vector& cr_followpos_ =
                        node_->followpos();
                    typename node_vector::const_iterator temp_iter_ =
                        cr_followpos_.begin();
                    typename node_vector::const_iterator temp_end_ =
                        cr_followpos_.end();

                    for (; temp_iter_ != temp_end_; ++temp_iter_)
                    {
                        node* cr_node_ = *temp_iter_;

                        if (cr_node_->end_state())
                            continue;

                        const index_set& nl_set_ =
                            set_mapping_[cr_node_->token()];

                        if (nl_set_.find(nl_id_) != nl_set_.end())
                        {
                            const node_vector& nl_followpos_ =
                                cr_node_->followpos();
                            typename node_vector::const_iterator nl_iter2_ =
                                nl_followpos_.begin();
                            typename node_vector::const_iterator nl_end2_ =
                                nl_followpos_.end();
                            bool found_ = false;

                            for (; nl_iter2_ != nl_end2_; ++nl_iter2_)
                            {
                                const node* nl_node_ = *nl_iter2_;
            ...
            const id_type nl_id_, const index_set_vector& set_mapping_)
        {
            ...
            if (set_.find(cr_id_) != set_.end())
            {
                const node_vector& cr_followpos_ =
                    node_->followpos();
                typename node_vector::const_iterator cr_iter_ =
                    cr_followpos_.begin();
                typename node_vector::const_iterator cr_end_ =
                    cr_followpos_.end();
                bool found_ = false;

                for (; cr_iter_ != cr_end_; ++cr_iter_)
                {
                    const node* cr_node_ = *cr_iter_;

                    if (cr_node_->end_state())
                        continue;

                    const index_set& cr_set_ = set_mapping_[cr_node_->token()];

                    if (cr_set_.find(nl_id_) != cr_set_.end())
                    {
                        const node_vector& nl_followpos_ =
                            cr_node_->followpos();
                        typename node_vector::const_iterator nl_iter_ =
                            nl_followpos_.begin();
                        typename node_vector::const_iterator nl_end_ =
                            nl_followpos_.end();
                        ...
                        if (found_)
                        {
                            ...
                            typename node_vector::iterator temp_iter_ = iter_;

                            ++temp_iter_;

                            while (temp_iter_ != end_)
                            {
                                node_ = *temp_iter_;
                                ...
                                {
                                    temp_iter_ = followpos_.erase(temp_iter_);
                                    end_ = followpos_.end();
                                    continue;
                                }
        ...
        static void check_zero_len(const rules& rules_,
            const node* root_)
        {
            if ((rules_.flags() & match_zero_len) == 0)
            {
                const node_vector& firstpos_ = root_->firstpos();
        ...
        {
            if (!(rules_.flags() & allow_suppressed_rules))
            {
        ...
        static std::string regex_from_idx(std::size_t idx_,
            const std_string_deque_deque& regexes)
        {
            typename std_string_deque_deque::const_iterator iter_ =
                regexes.begin();
            typename std_string_deque_deque::const_iterator end_ =
                regexes.end();
            ...
            std::set<id_type>& used_ids_)
        {
                ...
                iter_ != end_; ++iter_)
            {
                closure_ex(*iter_, end_state_, id_, user_id_, next_dfa_,
                    ...
                    vector_ptr_.get(), hash_, greedy_, flags_, used_ids_);
            }
            ...
            std::set<id_type>& used_ids_)
        {
            ...
            if (temp_end_state_)
            {
                if (!end_state_)
                {
                    ...
                    if (!(flags_ & allow_suppressed_rules))
